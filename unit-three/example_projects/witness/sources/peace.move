...
public struct Guardian<phantom T: drop> has key {
    id: UID,
}
...
/// code is called only once. With the Witness pattern it is
/// often the best practice.
fun init(witness: PEACE, ctx: &mut TxContext) {
    transfer::transfer(create_guardian(witness, ctx), ctx.sender())
}
